...
- この警告は .NET の `TextWriter` を経由しないため、**インプロセスのテスト(`CliTestHost.RunCliAsync`)では観測できない**。ただし「テストで捕捉不能」ではない: CLI を**実プロセスとして起動**すれば(`dotnet <テスト出力>/Recognizer.Cli.dll ...`)実 stderr(fd 2)を捕捉できる。回帰は `tests/Recognizer.Cli.Tests/ProcessSmokeTests.cs` が固定する(最終検証で、この 1 行を消しても全テストがグリーンのままになる穴が見つかったため追加した)。publish スモーク(§9.4)は手動の 1 回限りで、回帰ガードにはならない。
...
**stderr が 1 行の JSON であることを必ず検査する**(§8.1 の「OpenCV ネイティブログの抑止」の確認。なお回帰の**継続的な**検出は `ProcessSmokeTests`(実プロセス起動)が担う。publish スモークは配布形態そのものの確認):
...
| --- | --- | --- |
| `OutputType` | `Exe` | 実行可能 |
| `AssemblyName` | **設定しない**(`Recognizer.Cli` のまま) | 実装フェーズで判明: `AssemblyName = recognizer` は**実装不能**。.NET のアセンブリ ID は大文字小文字を区別せず、`recognizer` はライブラリの `Recognizer` と同一 ID とみなされて共存できない(実測: restore が `Ambiguous project name` で失敗 / 参照解決が壊れて `dotnet test` が通らない / publish しても実行時に `TypeLoadException`)。実行ファイル名 `recognizer` は、publish 後に apphost をリネームする MSBuild Target(`RenamePublishedHost`)で実現する |
| `RuntimeIdentifiers` | `linux-x64;win-x64;osx-arm64` | 要件 9.2。ライブラリと同じ 3 RID |
| `SelfContained` | `true`(**RID 指定時のみ**) | .NET ランタイム同梱(要件 9.1)。実装フェーズで判明: 無条件に置くと RID 未指定の `dotnet build` でも暗黙 RID が解決されて出力先が `bin/<Config>/net10.0/<rid>/` に移り、RID 無しでビルドされるテストプロジェクトから ProjectReference を解決できなくなる(`dotnet build` / `dotnet test` が壊れる)。`Condition="'$(RuntimeIdentifier)' != ''"` の PropertyGroup に隔離する |
| `PublishSingleFile` | `true`(**RID 指定時のみ**) | 単一実行ファイル(要件 9.1)。同上の理由で条件付き |
