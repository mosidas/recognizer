...
| `tests/Recognizer.Tests/Fixtures/`(*.onnx 7 種 + README 追記) | 新規/変更 | §9 の fixture ⑰〜㉓ |
...
- **fixture の増設**(research.md §4。生成スクリプト + 生成物コミット方式、既存 16 種のバイト列不変):
  ...
  - ㉒ `embed_unsupported_rank2_batch2`: 出力 `[2,4]`(rank2・先頭次元 ≠ 1)→ NotSupportedException(規則 e-d)。
  - ㉓ `embed_unsupported_dynamic_dim`: 出力 `[1,D]` の D が動的軸(≤0)→ NotSupportedException(規則 e-d)。
  ...
  - ⑳㉒㉓ は規則 e-d の非対応 4 分岐(複数出力=既存 `face_multi_output`・rank3 以上=⑳・rank2 先頭次元≠1=㉒・D 動的=㉓)を fixture で個別に行使するための集合(§6 (e-d) に対応)。
