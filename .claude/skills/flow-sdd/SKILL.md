---
name: flow-sdd
description: SDD(仕様駆動開発)ワークフロー。依頼を経路判定(ルーティング)して作業単位に分け、各単位を要件(dev-requirements)→設計(dev-design)→タスク分解(dev-decompose)→実装(dev-implement)の順に、承認ゲート付きの状態機械で駆動する composition。承認ゲートはすべて人間承認とする(自走・自己承認は、部品を直接束ねる拡張ワークフロー側で実現する)。「SDD で進めて」「仕様駆動で開発して」「要件から実装まで通しで」と頼まれたとき、または中断した SDD 作業を再開するときに使う。
---

# flow-sdd — SDD ワークフロー

部品を状態機械で束ね、依頼を作業単位(unit)へ振り分けて要件 → 設計 → タスク分解 → 実装まで駆動する composition。各フェーズの生成ロジックは部品の SKILL.md をそのまま実行し、この composition は**ルーティング・状態遷移・承認ゲート・フェーズ間の接続だけ**を担う。
...
```
initialized → requirements-generated →(gate: requirements)→ requirements-approved
→ design-generated →(gate: design)→ design-approved
→ tasks-generated →(gate: tasks)→ tasks-approved
→ implementing → completed(凍結)
差し戻し: design-generated→requirements-generated / tasks-generated→design-generated / implementing→tasks-generated
```
...
- main 等への統合(マージ・PR)は人間に委ねる。flow-sdd は自動マージしない。
- 経路 B(unit 化しない軽微変更)は現在のブランチで作業する。

## 4. 承認ゲート

すべての承認ゲート(ルーティング・requirements・design・tasks)は**人間承認**とする。承認は明示的に取り、沈黙・「お任せ」を承認と見なさない。実装(タスクループ・最終検証)は部品の設計どおり自走するが、`_Blocked:` 発生・最終検証 NO-GO・差し戻しが必要な状況では停止して人間に報告する。本番デプロイ等の不可逆操作はこのワークフローでは扱わない(dev-release とユーザーの明示承認に委ねる)。
...
- workdir に `state.json` があれば `<engine> status` で現在地を復元し、状態に対応するフェーズから続行する。`completed` なら凍結済みであることを伝えて停止する。roadmap があれば未完了の次 unit を案内する。
- 新規なら、ブランチ作成(3.)の後に `<engine> init --unit <unit>` で初期化する。

### Step 1: 要件フェーズ

1. `../dev-requirements/SKILL.md` の手順を workdir 上書きで実行する(ヒアリング → EARS 生成 → 内蔵ゲート → 保存・コミット)。
2. `<engine> set-state requirements-generated`。
3. ユーザーに requirements.md のレビューを依頼し、**明示的な承認を待つ**(沈黙・「お任せ」を承認と見なさない)。修正要望があれば同状態のまま反映して再提示する。
4. 承認を得たら `<engine> approve requirements`。state.json の更新は `chore(<unit>): requirements 承認に伴い state.json を更新` としてコミットする。

### Step 2: 設計フェーズ

1. `../dev-design/SKILL.md` の手順を実行する。
2. `<engine> set-state design-generated` → ユーザーの明示承認 → `<engine> approve design`。
3. 設計中に要件の不備が見つかったら、`<engine> set-state requirements-generated` で差し戻し、要件フェーズからやり直す。

### Step 3: タスク分解フェーズ

1. `../dev-decompose/SKILL.md` の手順を実行する。
2. `<engine> set-state tasks-generated` → ユーザーの明示承認 → `<engine> approve tasks`。
3. 設計の不備が見つかったら `<engine> set-state design-generated` で差し戻す。
...
4. 最終検証が **GO** になったら `<engine> set-state completed`(エンジンが中間生成物を凍結する)。`check.py` で error が無いことを確認し、state.json 更新をコミットして unit の完了を報告する。
5. roadmap がある場合は次の unit へ進む(各 unit の承認ゲートで停止しながら進める)。
