...
- **入力**: `<workdir>/tasks.md`(タスク定義。各タスクはタスク固有情報 = 説明・`_Requirements:` ID・`_Boundary:`・対象ファイル・検証コマンドを持ち、仕様の詳細は同じ workdir の `requirements.md` / `design.md` への参照で解決する)。
...
- 各タスクの仕様の詳細は、dev-implementer(新鮮なサブエージェント)が同じ workdir の `requirements.md`(`_Requirements:` の該当 ID)と `design.md`(該当節)を**直接読んで**解決する。サブエージェント内の読み込みなのでメイン文脈を消費しない。
- 参照先(該当 ID・該当節)が requirements.md / design.md に存在しない・不足する場合は、推測で補わず `NEEDS_CONTEXT` として上流の補強(dev-decompose での再分解、または対話での確認)を促す。
...
- **dev-implementer**(プロンプト: `./templates/implementer-prompt.md`): タスク固有情報・仕様の参照先(requirements.md の該当 ID・design.md の該当節)・Implementation Notes・注入知識を渡して実装させる。仕様の本文はプロンプトに転記せず、dev-implementer が参照先を読む。返却 `STATUS`: `READY_FOR_REVIEW` / `BLOCKED` / `NEEDS_CONTEXT`。
