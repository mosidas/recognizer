...
- port の 2 種類(差し替え port = 部品のデフォルト手順・データを置き換える / 知識 port = 作業内容に応じて注入する専門知識)は**概念上の区分**であり、仕組み(frontmatter・走査・選択)は共通。部品は差し替え port を `name` で参照する(例: dev-requirements は `name: hearing-questions` の port があればヒアリングを差し替える)。
