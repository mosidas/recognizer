...
EARS(Easy Approach to Requirements Syntax)は、要件をテスト可能で曖昧さのない構造化された文として記述する記法です。本スキル群では `requirements.md` の受け入れ基準(Acceptance Criteria)に用います。
...
各受け入れ基準の番号(例: `2.3`)は、`design.md` の Requirements Traceability 表と `tasks.md` の `_Requirements: 2.3_` 注記から参照される。要件番号を変更する際は下流ドキュメントとの整合に注意する。
